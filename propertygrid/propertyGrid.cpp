...
//JB 1.4: Made compile on C++ compiler
...
static COLORREF GetColor(LPTSTR src)
{
    ...
    RVal = GVal = BVal = 0;
    if (_tcslen(src) > 0)
        _stscanf_s(src, _T("%d,%d,%d"), &RVal, &GVal, &BVal);
...
#ifndef ToolTip_AddTool
#define ToolTip_AddTool(hwnd, ti) SendMessage(hwnd, TTM_ADDTOOL, 0, (LPARAM)ti);
#endif

#ifndef ToolTip_EnumTools
#define ToolTip_EnumTools(hwnd, idx, ti) SendMessage(hwnd, TTM_ENUMTOOLS, (WPARAM)idx, (LPARAM)ti);
#endif

#ifndef ToolTip_UpdateTipText
#define ToolTip_UpdateTipText(hwnd, ti) SendMessage(hwnd, TTM_UPDATETIPTEXT, 0, (LPARAM)ti);
#endif

#ifndef ToolTip_NewToolRect
#define ToolTip_NewToolRect(hwnd, ti) SendMessage(hwnd, TTM_NEWTOOLRECT, 0, (LPARAM)ti);
#endif
...
static LPITEMIDLIST ConvertPathToLpItemIdList(LPTSTR pszPath)
{
    ...
    if (SUCCEEDED(SHGetDesktopFolder(&pDesktopFolder)))
    {
		...
		MultiByteToWideChar(CP_ACP, 0, pszPath, -1, wszFullPath, MAX_PATH);

        pDesktopFolder->ParseDisplayName(
			NULL, NULL, wszFullPath, &chEaten, &pidl, &dwAttributes);
...
static VOID ListBox_OnLButtonDown(HWND hwnd, BOOL fDoubleClick, INT x, INT y, UINT keyFlags)
{
    ...
    {
		// JB_FEATURE_CHANGE: don't do this on double click
        //if (fDoubleClick) 
		// END_JB_FEATURE_CHANGE
        {
...
static HBRUSH Grid_OnCtlColorStatic(HWND hwnd, HDC hdc_, HWND hwndChild, INT type)
{
    ...
    if (NULL != g_lpInst->hwndPropDesc)
    {
        ...
        HDC hdc = GetDC(hwnd);
		RECT r = {0, g_lpInst->iVDivider - 2, WIDTH(rc), g_lpInst->iVDivider};
...
static VOID Grid_OnDrawItem(HWND hwnd, const DRAWITEMSTRUCT *lpDIS)
{
    ...
    if (PIT_CATALOG == pItem->iItemType)
    {
		...
		RECT r = {rectCatPart1.left + 6, rectCatPart1.top + 3, rectCatPart1.right - 6, rectCatPart1.bottom + 3};
    ...
    else
    {
		...
		RECT r = {rectPart1.left + 3, rectPart1.top + 3, rectPart1.right - 3, rectPart1.bottom + 3};
    ...
    if (PIT_CHECK != pItem->iItemType)
    {
        ...
        if (PIT_FONT == pItem->iItemType)
        {
            ...
            if (0 < PointSize)
            {
				...
				RECT r = {rectPart3.left + 3, rectPart3.top + 3,
        ...
        else
        {
			...
			RECT r = {rectPart3.left + 3, rectPart3.top + 3, rectPart3.right + 3, rectPart3.bottom + 3};
...
/// @param dwWidth The initial width of this control
/// @param dwHeight The initial height of this control
///
/// @returns HWND If the function succeeds, the grid handle, otherwise NULL.
HWND New_PropertyGrid(HWND hParent, DWORD dwTop, DWORD dwLeft, DWORD dwWidth, DWORD dwHeight)
{
    ...
    hPropertyGrid = CreateWindowEx(0, g_szClassName, _T(""),
         WS_CHILD | WS_TABSTOP, dwLeft, dwTop, dwWidth, dwHeight, hParent, (HMENU)1, hinst, NULL);

	ShowWindow(hPropertyGrid, SW_SHOW);
